...
class spot:
    ...
    def show(us, color, st):
        if us.closed == False :
            pygame.draw.rect(screen, color, (us.i * w, us.j * h, w, h), st)
            pygame.display.update()

    def path(us, color, st):
        pygame.draw.rect(screen, color, (us.i * w, us.j * h, w, h), st)
        pygame.display.update()

    def addNeighbors(us, grid):
        i = us.i
        j = us.j
        if i < cols-1 and grid[us.i + 1][j].obs == False:
            us.neighbors.append(grid[us.i + 1][j])
        if i > 0 and grid[us.i - 1][j].obs == False:
            us.neighbors.append(grid[us.i - 1][j])
        if j < row-1 and grid[us.i][j + 1].obs == False:
            us.neighbors.append(grid[us.i][j + 1])
        if j > 0 and grid[us.i][j - 1].obs == False:
            us.neighbors.append(grid[us.i][j - 1])
